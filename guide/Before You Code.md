...
Our repository is where we will put code relating to our project. How do we add and track code though you ask? Simple! Well, it is once you get used to it. We will be accessing GitHub through from our terminal and “pushing” code to our repository that way.
...
Once Homebrew is installed, you can install Git with the command `brew install git`. To verify that it is installed, try typing `git version` and if it tells you the version installed then it worked. Awesome! Now we are ready to clone our repository to our Desktop so that we can work on it and then push our changes back to GitHub. The good news is that you do not have to do this process every time you want to work on a project. This is really a one-time thing and now you have Homebrew and git installed on your machine for future use.

### Cloning Repositories
Let us clone our repository now! To do this, head back to GitHub and view your repository that we created. On the right side there should be a green button that says “Clone or Download”. Copy the URL given in the box and then in your terminal type `git clone https://github.com/MysteryPuppy/TestRepo.git` but replace my repository URL with your own. When you press enter the repository folder should download to your computer. Now if you type `ls` you will see there's a new folder on your Desktop called whatever you named your repository. To enter this folder, type `cd TestRepo` (where TestRepo is whatever your repository is called) and you are in.

<img src="/8.png" width=70% height=70% alt="Typing in commands into terminal.">

Now you are in the directory of the cloned repository. If you type `ls` again you'll see that it is empty aside from our README.md file. Let's get to adding files!
...
Back in your terminal, type `git fetch && git checkout working` to switch to the new branch called working. Now you are in the new branch. Let us change something. I'm going to add a new file with `touch NewFile.md`. Let us add this new file to the branch. To do this type `git add NewFile.md` then `git commit -m "adding a new file"` and finally type `git push origin working`. Remember to push to `Working` instead of `master` this time since we are not in the master branch.
...
That's headers! To write normal text you do not need to put anything special, just begin it on a new line.
